...
function EditCombo({ open, handleEdit, editData, menuData }) {
    ...
    const onSubmit = async (data) => {
        ...
        try {
            const response = await axios.put(`${menuProductAd}/${editData._id}`, { ...payload, image: profileImageUrl });
            handleEdit();
            menuData();
            reset({
                name: '',
    ...
    useEffect(() => {
        if (editData) {
            ...
            setProfileImageUrl(editData?.image); // Set profile image URL directly
            setImage(editData?.image); // Se
            // editData?.category?.map((item, i) => (
            //     setValue('')
            // ))

            editData?.category?.forEach((category, index) => {
                setValue(`category[${index}].productCategory`, { value: category.productCategory?._id, label: category.productCategory?.name });
                const products = category.products.map(product => ({ value: product?._id, label: product?.name }));
                setValue(`category[${index}].productName`, products);
            });
        }
    }, [editData, setValue]);
        ...
        <>
            ...
            >
                <div className="d-flex align-items-center justify-content-between mb-1 px-1" style={{ backgroundColor: '#e4510b', borderTopLeftRadius: '13px', borderTopRightRadius: '13px' }}>
                    <h4 className="modal-title" style={{ marginTop: '23px', marginBottom: '23px', paddingLeft: '10px', fontFamily: 'roboto', color: 'white' }}>Edit Menu Product</h4>
