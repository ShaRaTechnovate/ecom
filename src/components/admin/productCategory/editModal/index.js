...
function ProductCategoryEdit({ open, handleEdit, editData, additionalData }) {
    ...
    const submitForm = async (data) => {
        delete data.image;
        try {
            const response = await axios.put(`${productCategoryApi}/${editData._id}`, { ...data, image: profileImageUrl });
            handleEdit()
            additionalData()
    ...
    //         image.onload = async () => {
    //             if (image.width === 512 && image.height === 343) {
    //                 setImage(URL.createObjectURL(imageFile));
    //                 await uploadImageAndSetProfile(imageFile);
    //             } else {
    //                 toast.error('Image dimensions must be 512x342 pixels.');
    //             }
    //         };
